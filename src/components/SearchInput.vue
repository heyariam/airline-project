    ...
    <v-row align="center" justify="center" style="height:100%" dense>
      <v-col class="glassmorph" cols="12" lg="10" md="10" sm="6">
        ...
        <div class="searchbox">
          ...
          <div>
            <v-menu v-model="toDateMenu" :close-on-content-click="false" :nudge-right="40" lazy
              transition="scale-transition" offset-y full-width max-width="290px" min-width="290px">
              <template v-slot:activator="{ on }">
                <v-text-field label="Fecha retorno" readonly :value="toDateDisp" v-on="on"></v-text-field>
              </template>
              <v-date-picker locale="en-in" :min="minDate" :max="maxDate" v-model="toDateVal" no-title
